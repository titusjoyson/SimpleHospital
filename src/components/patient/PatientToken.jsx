...
import TokenModal from "../common/TokenModel";
...
export default function PatientToken() {
  ...
  const tokenFormConfig = useMemo(() => {
    return baseTokenFormConfig.map((section) => ({
      ...section,
      fields: section.fields.map((field) => ({ ...field, disabled: loading })),
    }));
  }, [loading]);
  ...
  const baseTokenFormButtonConfig = useMemo(
    () => [
      {
        text: "Save",
        fieldType: "submit",
  ...
  const tokenFormButtonConfig = useMemo(() => {
    return baseTokenFormButtonConfig.map((button) => ({
      ...button,
      disabled: loading,
    }));
  }, [baseTokenFormButtonConfig, loading]);
  ...
  return (
    <Paper elevation={0}>
      ...
      <TableCard
        ...
        rows={[
          [
            { label: "Patient Name", value: "Titus" },
            { label: "Patient Name", value: "Titus" },
            { label: "Patient Name", value: "Titus" },
          ],
          [
            { label: "Mobile Number", value: "999999999" },
            { label: "Mobile Number", value: "999999999" },
            { label: "Mobile Number", value: "999999999" },
          ],
        ]}
      ...
      <ConfigForm
        ...
        formButtonConfig={tokenFormButtonConfig}
        handleSubmit={handleSubmit}
        onSubmit={onSubmit}
        control={control}
        errors={errors}
